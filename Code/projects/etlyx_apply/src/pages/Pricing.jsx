...
const css = `
  ...
  .pricing-sub { color: #6b7280; font-size: 14px; text-align: center; margin-bottom: 48px; }
  .pricing-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; max-width: 680px; width: 100%; }
  .plan-card { background: #111318; border: 1px solid #1e2330; border-radius: 16px; padding: 32px 28px; display: flex; flex-direction: column; }
  .plan-card.featured { border-color: #c8a96e; position: relative; }
  ...
  .plan-features li { display: flex; align-items: flex-start; gap: 10px; color: #9ca3af; font-size: 13.5px; padding: 6px 0; border-bottom: 1px solid #1e2330; }
  .plan-features li:last-child { border-bottom: none; }
  .plan-features li.pro { color: #e8e4dc; }
  .feat-icon { flex-shrink: 0; font-size: 14px; margin-top: 1px; }
...
const FREE_FEATURES = [
  { text: "1 search source (Indeed)", locked: false },
  { text: "5 searches per month",     locked: false },
  { text: "Application tracker",      locked: false },
  { text: "Profile & resume storage", locked: false },
  { text: "Evaluator Agent scoring",  locked: true },
  { text: "All 4 search sources",     locked: true },
  { text: "Unlimited searches",       locked: true },
  { text: "Cover letter generation",  locked: true },
];

const PRO_FEATURES = [
  "LinkedIn, Indeed, hiring.cafe & Investment Banks",
  "Unlimited searches",
  "Evaluator Agent — full 5-criteria breakdown",
  ...
  return (
    <>
      ...
      <div className="pricing-wrap">
        <div className="pricing-logo">etlyx <span>apply</span></div>
        <div className="pricing-title">Simple, honest pricing</div>
        <div className="pricing-sub">Start free. Upgrade when you're ready to go all in.</div>
        ...
        <div className="pricing-grid">
          ...
          <div className="plan-card">
            ...
            <div className="plan-desc">Get started, no credit card needed</div>
            <ul className="plan-features">
              {FREE_FEATURES.map((f, i) => (
                <li key={i}>
                  <span className="feat-icon">{f.locked ? "🔒" : "✓"}</span>
                  <span style={{ color: f.locked ? "#4b5563" : undefined, textDecoration: f.locked ? "line-through" : undefined }}>{f.text}</span>
                </li>
              ))}
            </ul>
          ...
          <div className="plan-card featured">
            ...
            <div className="plan-desc">Everything you need to land the job</div>
            <ul className="plan-features">
